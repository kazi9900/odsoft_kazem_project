using Microsoft.Extensions.Configuration;
using PC4MT.BLL.Services.Interfaces;
using System;
using System.Collections.Generic;
using System.Net.Mail;

namespace PC4MT.BLL.Services
{
    public class MailService : IMailService
    {
        private IConfiguration _configuration;


        public MailService(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        public bool SendEmail(string subject, string from, List<String> to, String message)
        {
            try
            {
                MailMessage mail = new MailMessage();
                SmtpClient SmtpServer = new SmtpClient("smtp.gmail.com");
                if (from == null)
                {
                    from = "glossloaderservice@gmail.com"; //_configuration.GetSection("SMTP:From").Value;
                }
                mail.From = new MailAddress(from);
                to.ForEach(x => mail.To.Add(x));
                mail.Subject = subject;
                mail.IsBodyHtml = true;
                mail.Body = message;

                SmtpServer.Port = 587;
                SmtpServer.UseDefaultCredentials = false;
<<<<<<< HEAD
                SmtpServer.Credentials = new System.Net.NetworkCredential("glossloaderservice@gmail.com", "u q z l o e v c u w u q b t b f");
=======
                SmtpServer.Credentials = new System.Net.NetworkCredential("glossloaderservice@gmail.com", "l y e m z p c y a qd n k z v z");
>>>>>>> c73bda7d6bfcc629c075700fcec7fa98de9b2d37
                SmtpServer.EnableSsl = true;

                SmtpServer.Send(mail);
                return true;
            }
            catch (Exception) { throw; }
        }
    }
}
